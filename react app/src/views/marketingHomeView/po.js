...
import {
    Table, Input, Button, Pagination, Radio, Progress, Row, Col, Card, Drawer, message, Menu, Modal, Dropdown
} from 'antd';
import {MoreOutlined} from "@ant-design/icons"
import POCancel from '../../components/PO/poCancel'
import POTerminate from '../../components/PO/poTerminate'
...
//total: res.counts.total,
    ...
    setData=(data)=>{
        this.setState({submission_id: data.obj.submission, view: true})
    }
    handleSelect = key => {
    ...
    render() {
        ...
        const terminate_menu = (status, data, project, i) => (
            <Menu>

                <Menu.Item key="0" onClick={() => {
                    this.setState({poId: data.id})
        ...
        const cancel_menu = (status, data, project, i) => (
            <Menu>
        ...
        const columns = [
                {
                    ...
                    width: '12%',
                    render: (text, record) => (
                        <span onClick={()=>this.setData(record)}>
                            {text}
                        </span>
                    )
                },
                              ...
                              }
                                {
                                    record.check_list.status ?
                                        <Button disabled={!(record.marketer.toLowerCase() === marketer.toLowerCase())}
                                                ...
                                                onClick={() => this.mailToOnboarding(record.id)}>
                                            Mail to Onboarding</Button> : null
                                        }
                ...
                },
                {
                    ...
                    render: (text, record, i) => (
                        ...
                        (text === 'new' || text === 'on_boarded' || text === 'received')
                            ?
                            <Dropdown trigger={['click']} overlay={cancel_menu(text, record, record.project, i)}>
                                <MoreOutlined style={{marginLeft: 70}}/>
                            </Dropdown>
                            :
                            (text === 'joined' || text === 'signed') ?
                                <Dropdown trigger={['click']} overlay={terminate_menu(text, record, record.project, i)}>
                                    <MoreOutlined style={{marginLeft: 70}}/>
                                </Dropdown> :
                                null
